...
const PostView = (props) => {
	...
	const findLike = (id) => {
		axios
			.get(`/api/users/${props.auth.user.id}/users/likes/${id}`)
			.then((res) => {
				setLike(res.data);
			})
			.catch((err) => {
	...
	const renderLikes = (postId) => {
		axios.get(`/api/likes/${postId}`).then((res) => {
	...
	const likePost = (postId) => {
		axios
			.post(`/api/users/${props.userId}/likes`, { post_id: postId, user_id: props.auth.user.id })
			.then((res) => {
				setLike(res.data);
				renderLikes(postId);
			})
	...
	const unlikePost = (postId) => {
		axios.delete(`/api/users/${props.userId}/users/likes/${postId}`).then((res) => {
			setLike(null);
			renderLikes(postId);
	...
	const editSinglePost = (id, post) => {
		...
		axios
			.put(
				`/api/users/${post.user_id}/posts/${id}?title=${post.title}&description=${post.description}&board_id=${post.board_id}`,
				data
			)
			.then((res) => {
				setCard(res.data);
	...
	const deletePost = (id) => {
		axios.delete(`/api/boards/${card.board_id}/posts/${id}`).then((res) => {
			...
			<div className="post-info">
				{props.auth.user.id === card.user_id ? (
					<div>
						...
						<img className="small-icon" src={trashIcon} onClick={() => deletePost(card.id)} />{' '}
					</div>
				) : null}
				...
				<div className="like-container">
					<div className="comment-container">
						...
						{like ? (
							<button className="like-button" onClick={() => unlikePost(card.id)}>
								Unlike
							</button>
						) : (
							<button className="like-button" onClick={() => likePost(card.id)}>
								<img className="small-icon" src={likeIcon} />Like
							</button>
						)}
					</div>
					<div className="comment-container">
...
const ConnectedPostView = (props) => {
	return <AuthConsumer>{(auth) => <PostView {...props} auth={auth} />}</AuthConsumer>;
};
