...
const Posts = (props) => {
	...
	useEffect(() => {
		axios.get(`/api/boards/${props.boardId}/posts`).then((res) => {
			setPosts(res.data);
			getFollows();
			followed();
		});
	...
	const getFollows = () => {
		axios.get(`/api/user/user_boards`).then((res) => {
			setFollows(res.data);
		});
	};
	...
	const editPost = (id, post) => {
		...
		axios
			.put(
				`/api/users/${post.user_id}/posts/${post.id}?title=${post.title}&description=${post.description}&board_id=${post.board_id}`,
				data
			)
			.then((res) => {
	...
	const removePost = (id) => {
		axios.delete(`/api/boards/${props.boardId}/posts/${id}`).then((res) => {
	...
	return (
		<div>
			...
			<br />
			<br />
			<div className="card-grid">{renderPosts()}</div>
		</div>
	);
};

const ConnectedPosts = (props) => <AuthConsumer>{(auth) => <Posts {...props} auth={auth} />}</AuthConsumer>;
