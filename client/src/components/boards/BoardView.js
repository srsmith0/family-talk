...
const BoardView = (props) => {
	...
	const removeBoard = (id) => {
		axios.delete(`/api/boards/${id}`).then((res) => {
	...
	const editSingleBoard = (id, board) => {
		...
		axios
			.put(`/api/boards/${id}?name=${board.name}&description=${board.description}&public=${board.public}`, data)
			.then((res) => {
				setBoard(res.data);
			});
	};
		...
		return (
			<div className="main-container">
				<div className="board-container">
					<h1 className="board-title">{board.name}</h1>
					<p className="board-description">{board.description}</p>
					{props.location.following ? (
						<button onClick={() => props.location.handleUnfollow(board.id)}>Unfollow</button>
					) : null}
					<button onClick={props.history.goBack}>Go Back</button>
					<Posts
...
const ConnectedBoardView = (props) => {
	return <AuthConsumer>{(auth) => <BoardView {...props} auth={auth} />}</AuthConsumer>;
};
