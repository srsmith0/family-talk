...
const AddPost = (props) => {
	...
	const theBoards = () => {
		return boards.map((b) => <option value={b.id}> {b.name}</option>);
	};
	...
	const handleSubmit = (e) => {
		...
		axios
			.post(
				`/api/users/${props.auth.user
					.id}/posts?title=${title}&description=${description}&board_id=${boardChoice}`,
				data
			)
			.then((res) => {
...
const ConnectedAddPosts = (props) => (
	<AuthConsumer>
		{(auth) => <BoardConsumer>{(board) => <AddPost {...props} auth={auth} board={board} />}</BoardConsumer>}
	</AuthConsumer>
);
